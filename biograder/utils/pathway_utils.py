    ...
    if database.lower() == "reactome":
        ...
        for protein in proteins:
            ...
            if protein in df.index:
                ...
                if not quiet:
                    warnings.warn(f"The protein '{protein}' was not found in the WikiPathways data.", ParameterWarning,
                                  stacklevel=2)
    else:
...
def get_proteins_in_pathways(pathways, database, quiet=False):
    ...
    if database.lower() == "reactome":
        ...
        for pathway_id in pathways:
            ...
            if resp.status_code == 404 or (resp.status_code == requests.codes.ok and (
                    len(resp.content.decode("utf-8")) == 0 or len(resp.json()) == 0)):
                if not quiet:
                    warnings.warn(
        ...
        for pathway in pathways:
            ...
            if pathway in df.columns:
                ...
                if not quiet:
                    warnings.warn(f"The pathway '{pathway}' was not found in the WikiPathways data.", ParameterWarning,
                                  stacklevel=2)
...
def reactome_pathway_overlay(pathway, df=None, analysis_token=None, open_browser=True, export_path=None,
                             image_format="png", display_col_idx=0, diagram_colors="Modern", overlay_colors="Standard",
                             quality=7):
    ...
    if df is not None:
        ...
        params = {"pageSize": "0",
                  "page": "1"}  # We only need the analysis token, so set pageSize to 0 so we don't worry about getting any of the data for individual pathways.
...
def reactome_enrichment_analysis(analysis_type, data, sort_by, ascending, include_high_level_diagrams=True,
                                 disease_pathways=True, include_interactors=False):
